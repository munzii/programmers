...
SELECT H.HISTORY_ID,
CASE 
    WHEN ABS(DATEDIFF(H.END_DATE, H.START_DATE)) + 1 < 7
    THEN FLOOR(C.DAILY_FEE * (DATEDIFF(H.END_DATE, H.START_DATE) + 1))
    WHEN ABS(DATEDIFF(H.END_DATE, H.START_DATE)) + 1 < 30
    THEN FLOOR((C.DAILY_FEE * 0.95) * (DATEDIFF(H.END_DATE, H.START_DATE) + 1))
    WHEN ABS(DATEDIFF(H.END_DATE, H.START_DATE)) + 1 < 90
    THEN FLOOR((C.DAILY_FEE * 0.92) * (DATEDIFF(H.END_DATE, H.START_DATE) + 1))
    ELSE FLOOR((C.DAILY_FEE * 0.85) * (DATEDIFF(H.END_DATE, H.START_DATE) + 1))
    END AS FEE
